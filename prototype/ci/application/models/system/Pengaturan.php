...
class Pengaturan extends CI_Model {
	...
	private function __hapus_notif()
	{
		...
		try{
			...
			Saya\Notif::set( 2 , 'Gagal Bersihin Notif Log', Model\User::$data['userid'] );
			if(($set['sys_debug_db'])){
				return self::$_hasil=array('error'=> $e->getMessage() );
			}else return self::$_hasil=array('error'=> 'Ada Kesalahan !!!' );
		}
		Saya\Notif::set( 1 , 'System Log Has Been Clear', Model\User::$data['userid'] );
	...
	private function __hapus(){
		...
		if($save){
			Saya\Notif::set( 1 , ' System\'s Background Removed ', Model\User::$data['userid'] );
	...
	private function __set_background(){
		...
		Upload::$_folder = 'upload/profile/'; Upload::$_max_width=1200;
		...
		if($save){
			Saya\Notif::set( 1 , ' Set Background ', Model\User::$data['userid'] );
	...
	private function __simpan(){
		...
		if( $save ){
			Saya\Notif::set( 1 , ' Settingan System' , Model\User::$data['userid'] );
