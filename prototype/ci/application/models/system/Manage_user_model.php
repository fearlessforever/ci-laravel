...
class Manage_user_model extends CI_Model {
	...
	private function __remove_app_permission(){
		...
		try{
			$save = Saya\DB::table( KODE .'pengguna_izin')->limit(1)->where('nama_app' , $app)->where('level', $lvl)->delete();
	...
	private function __add_app_permission(){
		...
		$set = Saya\Template::getData();
		$pdo = Saya\DB::connection()->getPdo()->prepare("INSERT INTO ". KODE ."pengguna_izin(level,nama_app) VALUES(:level,:nama_app) ");
	...
	private function __remove_user(){
		...
		if( $lvl== Model\User::$data['userid'] ){
			self::$_hasil =array('error'=>'Tidak Bisa Menghapus/Block userid Sendiri'); return;
		}
		$save=null;
		$set = Saya\Template::getData();
		...
		try{
			if( empty($block) ){
				$save = Saya\DB::table(KODE .'pengguna')->where('userid',$lvl )->limit(1)->delete();
			}else{
				$save = Saya\DB::table(KODE .'pengguna')->where('userid',$lvl )->limit(1)->update( array('blokir'=> ($block == 'block')? 'Y' : 'N') );
			}
		}catch(PDOException $e){
	...
	private function __add_user(){
		...
		if( !Validasi::_array($data,'username|email|passnya|nama|level') ){
			self::$_hasil =array('error'=>'User id,email,password,nama , dan Level Tidak Boleh Kosong !!!'); return;
		}
		if ( !filter_var( $data['email'] , FILTER_VALIDATE_EMAIL)) {
			self::$_hasil =array('error'=> $data['email'].' Bukan email yang valid !!!'); return;
		}
		
		$data['passnya'] = password_hash($data['passnya'] , PASSWORD_DEFAULT );
		...
		$set = Saya\Template::getData();
		try{
			$save = Saya\DB::table(KODE .'pengguna')->insertGetId( $data );
		}catch(PDOException $e){
			self::$_hasil= array('error' => empty($set['sys_debug_db']) ? '[DB] Ada Kesalahan di simpan data' : $e->getMessage() ); return;
		}
		...
		if($save){
			self::$_hasil=array(
				...
				,'baru'=>array(
					'userid'=>$save,'username'=>$data['username'],'email'=>$data['email'],'level'=>$data['level'],'nama'=>$data['nama'],'buat'=>date('d F Y ~ H:i'),'block'=>'N'
				)
			);
			Saya\DB::insert("INSERT IGNORE INTO ". KODE ."pengguna_ext(userid,nama,isi) VALUES(:userid,:nama,:isi ) ",  array('userid'=>$save ,'nama'=>'folder','isi'=>'') );
			Saya\DB::insert("INSERT IGNORE INTO ". KODE ."pengguna_ext(userid,nama,isi) VALUES(:userid,:nama,:isi ) ",  array('userid'=>$save ,'nama'=>'profile_pic','isi'=>'noimage.jpg') );
	...
	private function __remove_level(){
		...
		try{
			$save = Saya\DB::table(KODE .'pengguna_lvl')->where('level',$lvl )->limit(1)->delete();
		}catch(PDOException $e){
			self::$_hasil= array('error' => empty($set['sys_debug_db']) ? '[DB] Ada Kesalahan di Database' : $e->getMessage() ); return;
		}
	...
	private function __add_level(){
		...
		$set = Saya\Template::getData();
		try{
			$save = Saya\DB::table(KODE .'pengguna_lvl')->insert( $data );
		}catch(PDOException $e){
			self::$_hasil= array('error' => empty($set['sys_debug_db']) ? '[DB] Ada Kesalahan di simpan data' : $e->getMessage() ); return;
		}
	...
	private function __view(){
		...
		if( empty($opsi) || $opsi =='level'){
			$core = Saya\DB::table( KODE .'pengguna_lvl')->select('level','ket','bintang') ->limit( $limit );
			if($cari){
		...
		if( empty($opsi) || $opsi =='user'){
			$core = Saya\DB::table( Saya\DB::raw( KODE .'pengguna a') )
						->select( Saya\DB::raw("a.userid,a.username,a.email,b.ket,a.nama,DATE_FORMAT(a.buat,'%d %M %Y ~ %H:%i') as buat,a.blokir as block") )
						->leftJoin( Saya\DB::raw( KODE .'pengguna_lvl b') , 'b.level','=', 'a.level')
						->limit( $limit ) ;
			if($next > 0){
				$offset = ($next - 1) * $limit;
				$core->skip( $offset )->take( $limit );
		...
		if( empty($opsi) || $opsi =='izin'){
			$core = Saya\DB::table( Saya\DB::raw( KODE .'pengguna_izin a') )
		...
		if(empty($next)){
			self::$_hasil['total_level']= Saya\DB::table( KODE .'pengguna_lvl')->count() ;
			self::$_hasil['total_user']= Saya\DB::table( KODE .'pengguna')->count() ;
			self::$_hasil['total_izin']= Saya\DB::table( KODE .'pengguna_izin')->count() ;
