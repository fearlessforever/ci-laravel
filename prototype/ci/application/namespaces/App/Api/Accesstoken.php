...
Class Accesstoken extends \App\Api\Abs\AbsAccessToken{
	...
	{
		try{
			$this->access_token_string = !empty($accessToken) ?  $accessToken : $this->CI->input->get('accesstoken') ;
			$this->sess_read();
		}catch( Exception $e){
	...
	function /* _readAccessToken */ sess_read()
	{
		...
		if ( ! is_array($session) OR ! isset($session['session_id']) OR ! isset($session['ip_address']) OR  ! isset($session['at_generate']) OR  ! isset($session['userid']))
		{
			throw new Exception(' Access Token is not Valid ');
		}
